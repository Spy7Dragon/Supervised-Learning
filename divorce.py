...
divorce_columns = ['MAR', 'MARHT', 'SEX', 'SCHL', 'OCCP', 'PINCP']
feature_columns = ['SEX', 'SCHL', 'OCCP', 'PINCP']


def create_divorce_data():
    raw_data1 = pd.read_csv('raw_data/psam_pusa.csv', usecols=divorce_columns)
    raw_data2 = pd.read_csv('raw_data/psam_pusb.csv', usecols=divorce_columns)
    raw_data = raw_data1.append(raw_data2)
    # Remove never married and people marrying more than once
    raw_data = raw_data[raw_data.MAR != 5]
    raw_data = raw_data[raw_data.MARHT == 1]
    ...
    train_classes = (train.MAR != 3).to_frame('Married')
    test_classes = (test.MAR != 3).to_frame('Married')

    train_features.to_csv('data/divorce_train_features.csv')
    test_features.to_csv('data/divorce_test_features.csv')
    train_classes.to_csv('data/divorce_train_classes.csv')
    test_classes.to_csv('data/divorce_test_classes.csv')
